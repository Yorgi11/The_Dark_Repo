...
public class Enemy : MonoBehaviour
{
    ...
    private void Update()
    {
        if (seenPlayer == true)
        {
            if (navigation.velocity.magnitude > speed * 1.5f) navigation.velocity = navigation.velocity.normalized * speed * 1.5f;
            if (navigation.velocity.magnitude <= 0.05f) targ = transform.position + new Vector3(Random.Range(0f, maxrange), 1f, Random.Range(0f, maxrange));
